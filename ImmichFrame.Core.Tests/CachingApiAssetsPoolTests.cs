...
public class CachingApiAssetsPoolTests
{
    ...
    public void Setup()
    {
        _mockApiCache = new Mock<ApiCache>(null, null); // ILogger, IOptions<AppSettings>
        _mockImmichApi = new Mock<ImmichApi>(null, null, null); // ILogger, IHttpClientFactory, IOptions<AppSettings>
    ...
    public async Task GetAssets_ReturnsRequestedNumberOfAssets()
    {
        ...
        Assert.IsTrue(result.All(a => a.Type == AssetTypeEnum.IMAGE));
    }
    ...
    public async Task AllAssets_UsesCache_LoadAssetsCalledOnce()
    {
        ...
        // Assert
        Assert.AreEqual(1, loadAssetsCallCount, "LoadAssets should only be called once.");
    }
    ...
    public async Task ApplyAccountFilters_FiltersArchived()
    {
        ...
        // Assert
        Assert.IsFalse(result.Any(a => a.Id == "3"));
        Assert.AreEqual(3, result.Count); // 1, 4, 5
    }
    ...
    public async Task ApplyAccountFilters_FiltersImagesUntilDate()
    {
        ...
        // Expected: Asset "1", "5"
        Assert.IsTrue(result.All(a => a.ExifInfo.DateTimeOriginal <= untilDate));
        Assert.AreEqual(2, result.Count, string.Join(",", result.Select(x=>x.Id)));
        Assert.IsTrue(result.Any(a=>a.Id == "1"));
        Assert.IsTrue(result.Any(a=>a.Id == "5"));
    }
    ...
    public async Task ApplyAccountFilters_FiltersImagesFromDate()
    {
        ...
        // Expected: Asset "3", "4"
        Assert.IsTrue(result.All(a => a.ExifInfo.DateTimeOriginal >= fromDate));
        Assert.AreEqual(2, result.Count, string.Join(",", result.Select(x=>x.Id)));
        Assert.IsTrue(result.Any(a=>a.Id == "3"));
        Assert.IsTrue(result.Any(a=>a.Id == "4"));
    }
    ...
    public async Task ApplyAccountFilters_FiltersRating()
    {
        ...
        Assert.IsTrue(result.All(a => a.ExifInfo.Rating == 5));
        Assert.AreEqual(2, result.Count, string.Join(",", result.Select(x=>x.Id)));
        Assert.IsTrue(result.Any(a=>a.Id == "1"));
        Assert.IsTrue(result.Any(a=>a.Id == "4"));
    }
    ...
    public async Task ApplyAccountFilters_CombinedFilters()
    {
        ...
        Assert.IsTrue(result.Any(a => a.Id == "1"));
        Assert.IsTrue(result.Any(a => a.Id == "4"));
        Assert.IsFalse(result.Any(a => a.Id == "3" || a.Id == "5" || a.Id == "2"));
    }
}
