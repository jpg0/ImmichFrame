...
public class AllAssetsPoolTests
{
    ...
    public void Setup()
    {
        ...
        _mockApiCache.Setup(c => c.GetOrAddAsync(
                It.IsAny<string>(),
                It.IsAny<Func<Task<AssetStatisticsResponseDto>>>() // For GetAssetCount
            ))
            .Returns<string, Func<Task<AssetStatisticsResponseDto>>>(async (key, factory) => await factory());
    }
    ...
    {
        // Arrange
        var stats = new AssetStatisticsResponseDto { Images = 100 };
        ...
        _mockApiCache.Verify(cache => cache.GetOrAddAsync(nameof(AllAssetsPool), It.IsAny<Func<Task<AssetStatisticsResponseDto>>>()), Times.Once);
    }
    ...
    public async Task GetAssets_CallsSearchRandomAsync_WithCorrectParameters()
    {
        ...
        // Assert
        Assert.AreEqual(requestedCount, assets.Count());
    ...
    public async Task GetAssets_ExcludesAssetsFromExcludedAlbums()
    {
        ...
        Assert.IsFalse(result.Any(a => a.Id == "excluded1"));
        Assert.IsTrue(result.All(a => a.Id.StartsWith("main")));
