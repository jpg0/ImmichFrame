...
{
    private Mock<ILogger<QueuingAssetPool>> _mockLogger;
    ...
    public void Setup()
    {
        _mockLogger = new Mock<ILogger<QueuingAssetPool>>();
        ...
        _queuingAssetPool = new QueuingAssetPool(_mockLogger.Object, _mockDelegatePool.Object);
    }
    ...
    public async Task GetNextAsset_RetrievesFromInitiallyEmptyQueue_TriggersReload_ReturnsAsset()
    {
        ...
        Assert.AreEqual(assetToReturn.Id, result.Id);
    ...
    public async Task GetNextAsset_QueueAboveThreshold_DoesNotTriggerReload()
    {
        ...
        // Assert
        Assert.IsNotNull(result);
        Assert.AreEqual(initialAssets.First().Id, result.Id);
    ...
    public async Task ReloadAssetsAsync_PreventsConcurrentReloads()
    {
        ...
        _mockDelegatePool.Setup(dp => dp.GetAssets(ReloadBatchSize, It.IsAny<CancellationToken>()))
            .ReturnsAsync(() => {
                ...
                Task.Delay(100).Wait();
                return CreateSampleAssets(1, "slow_load");
            });
        ...
        var task1 = _queuingAssetPool.GetNextAssetForTesting(CancellationToken.None);
        // Small delay to ensure the first GetNextAsset's ReloadAssetsAsync has started and acquired semaphore
        await Task.Delay(10);
        var task2 = _queuingAssetPool.GetNextAssetForTesting(CancellationToken.None);
        ...
        // Check logs for "Assets already being loaded" if possible, or assume semaphore works as intended.
        // Direct verification of this log is tricky without intercepting ILogger.
        // The primary check is that GetAssets on delegate is called only once.
    }
    ...
    public async Task GetNextAsset_HandlesOperationCanceledException_ReturnsNull()
    {
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("Read asset list timed out")),
                null,
    ...
    public async Task ReloadAssetsAsync_AddsFetchedAssetsToQueue()
    {
        ...
        Assert.AreEqual(assetsToLoad.Count, retrievedAssets.Count, "Should retrieve all assets loaded by ReloadAssetsAsync.");
        foreach(var loadedAsset in assetsToLoad)
        {
            Assert.IsTrue(retrievedAssets.Any(ra => ra.Id == loadedAsset.Id), $"Asset {loadedAsset.Id} not found in retrieved assets.");
        }
    }
}
