...
public class PersonAssetsPoolTests // Renamed from PeopleAssetsPoolTests to match class name
{
    ...
    private SearchResultResponseDto CreateSearchResult(List<AssetResponseDto> assets, int total) =>
        new SearchResultResponseDto { Assets = new AssetListResponseDto { Items = assets, Total = total } };
    ...
    public async Task LoadAssets_CallsSearchAssetsForEachPerson_AndPaginates()
    {
        ...
        // Assert
        Assert.AreEqual(batchSize + 30 + 20, result.Count);
        Assert.IsTrue(result.Any(a => a.Id == "p1_p1_0"));
        Assert.IsTrue(result.Any(a => a.Id == "p1_p2_29"));
        Assert.IsTrue(result.Any(a => a.Id == "p2_p1_19"));
    ...
    public async Task LoadAssets_PersonHasNoAssets_DoesNotAffectOthers()
    {
        ...
        Assert.IsTrue(result.All(a => a.Id.StartsWith("p1_")));
    }
}
